  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          productId: isConsultation ? 'prod-consultation' : 'prod-main-book',
          title: productTitle,
          price: finalPrice,
          customerEmail: email,
          customerName: name,
  ...
  return (
    <div className="min-h-screen bg-brand-black" dir="rtl">
      <div className="max-w-5xl mx-auto px-4 py-8 lg:py-12">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-brand-gold transition mb-8"
        >
              ...
              <form onSubmit={handlePayment} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-brand-black border border-brand-white/10 rounded-xl py-3.5 px-4 text-white placeholder-gray-600 focus:outline-none focus:border-brand-purple transition text-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    dir="ltr"
                    className="w-full bg-brand-black border border-brand-white/10 rounded-xl py-3.5 px-4 text-white placeholder-gray-600 focus:outline-none focus:border-brand-purple transition font-mono text-sm text-left"
                  />
                  <p className="text-[10px] text-gray-600 mt-1">سيتم إرسال روابط التنزيل وبيانات الدخول على هذا البريد</p>
                </div>

                {/* Coupon */}
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-2">رمز الخصم (اختياري)</label>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="COUPON"
                      dir="ltr"
                      className="flex-1 bg-brand-black border border-brand-white/10 rounded-xl py-3 px-4 text-white placeholder-gray-600 focus:outline-none focus:border-brand-purple transition font-mono text-sm text-left"
                    />
                    <button
                      type="button"
                ...
                <div className="border-t border-brand-white/5 pt-5">
                  <label className="block text-xs font-bold text-gray-400 mb-3">طريقة الدفع</label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <CreditCard className={`w-5 h-5 mx-auto mb-1 ${paymentMethod === 'stripe' ? 'text-brand-purple-light' : 'text-gray-500'}`} />
                      <span className={`text-xs font-bold ${paymentMethod === 'stripe' ? 'text-white' : 'text-gray-400'}`}>
                        بطاقة ائتمان
                      </span>
                      <span className="block text-[10px] text-gray-500 mt-0.5">Visa, Mastercard, Amex</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <span className={`text-lg font-bold ${paymentMethod === 'paypal' ? 'text-brand-gold' : 'text-gray-500'}`}>P</span>
                      <span className={`text-xs font-bold block ${paymentMethod === 'paypal' ? 'text-white' : 'text-gray-400'}`}>
                        PayPal
                      </span>
                      <span className="block text-[10px] text-gray-500 mt-0.5">حساب أو بطاقة</span>
                    </button>
                  </div>
                </div>
                ...
                {/* Trust Micro */}
                <div className="flex flex-wrap justify-center gap-4 text-[10px] text-gray-500 pt-2">
            ...
            <div className="glass-gold rounded-3xl p-6 sticky top-24">
              ...
              <div className="flex gap-3 mb-6 pb-6 border-b border-brand-white/5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-gray-500">وصول فوري ودائم</p>
                </div>
              </div>

              {/* What's included */}
              <div className="space-y-2 mb-6 pb-6 border-b border-brand-white/5">
                <span className="text-[10px] font-bold text-gray-500 uppercase">يتضمن:</span>
                {[
              ...
              <p className="text-center text-[10px] text-gray-500">4.9/5 من 127 تقييم</p>
            </div>
          </div>
        </div>
      </div>
